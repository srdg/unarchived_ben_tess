...
lang=ben
rm -rf ./tesstrain-master/data/ben-ground-truth && mkdir -p ./tesstrain-master/data/ben-ground-truth
set -- "$SOURCE"*.png
for img_file; do
    echo -e  "\r\n File: $img_file"
    OMP_THREAD_LIMIT=1 tesseract  "${img_file}" "${img_file%.*}"  --psm 6  --oem 1  -l $lang ben page_separator='' hocr
    PYTHONIOENCODING=UTF-8 ./hocr-tools-master/hocr-extract-images -b ./010_LetterPress-Data/ -p ./tesstrain-master/data/ben-ground-truth/"${img_file%.*}"-%03d.exp0.tif  "${img_file%.*}".hocr 
done
